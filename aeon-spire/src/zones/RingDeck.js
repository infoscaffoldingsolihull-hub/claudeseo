  ...
  massing() {
    ...
    this.shell.add(mesh(
      loft((t, y) => {
        ...
        /* Hold the waist across almost the whole disc and flare only in the
           last few metres. At an exponent of 0.7 the shaft was back to full
           width well inside the disc's tip, where the disc itself has
           tapered away, so it read as a dark notch bitten out of the coin. */
        const kk = Math.pow(k, 0.22);
        const hx = lerp(endX, waist, kk);
        const hz = lerp(endZ, waist * 0.94, kk);
