...
export function createTours(ctx) {
  ...
  const HOUSE = [
    beat([0, 1.7, 13.6], [0, 2.2, 6], 7,
      'Bagh-e-Shahi Manor stands on two kanals — 1,020 square metres — in DHA Phase VI, Lahore. Eight hundred and ninety-four square metres of covered area over a basement, a ground floor and a first floor, behind a two-storey portico.'),
  ...
  const CONSTRUCTION = [
    ...
    beat([-22, 8, 22], [-2, 0, -2], 6,
      'Site establishment. The boundary wall goes up first: it secures the site, and pilferage is a quantified risk on this register with a two point one crore impact.', Math.round(finish * 0.12)),
    beat([-18, 7, 14], [-2, -2, -4], 6,
      'Bulk excavation for the basement and the footings. Fourteen hundred cubic metres out, machine-dug and carted within fifteen kilometres.', Math.round(finish * 0.18)),
    ...
    beat([-20, 9, 14], [-2, 3, -3], 6,
      'Masonry. Ground floor first, first floor overlapping it by six days on a finish-to-start with a negative lag. The house is bare brick at this stage: the cladding and the plaster are different packages and have not been paid for yet.', Math.round(finish * 0.62)),
    beat([-14, 6, 12], [-2, 4, -2], 6,
      'Façade. Sandstone cladding dry-fixed on stainless anchors, the portico order set, the parapet and the cupola built. Forty working days of the longest single activity on the project.', Math.round(finish * 0.70)),
