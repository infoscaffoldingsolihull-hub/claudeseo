...
export class InteriorSystem {
  ...
  _build() {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      rough.push(box(1.5, 0.53, hallD - 0.6 - i * 1.1, AXIS_X + 1.75, top - 0.265, hallZ + 0.3 + i * 0.55));
      colliders.push({
        // Off the line you walk in on. Hard against the entry line, this flight
        // was climbed by accident on the way to the passage down.
        minX: AXIS_X + 1.0, maxX: AXIS_X + 2.5,
        minY: top - 0.6, maxY: top,
        minZ: hallZ - hallD / 2 + 0.3 + i * 1.1, maxZ: hallSouthZ,
        tag: 'junction-ledge',
      });
    }
    // Bedrock under the knot.  Three passages and a room meeting in one place
    // leaves corners where a player can slip between two shells, and below a
    // tomb there is nothing to land on.  There is solid rock here in reality,
    // so there is solid rock here now, and the fall becomes a stumble.
    colliders.push({
      minX: AXIS_X - 7, maxX: AXIS_X + 7,
      minY: hallFloorY - 2.4, maxY: hallFloorY - 0.02,
      minZ: hallNorthZ - 6, maxZ: hallSouthZ + 6, tag: 'bedrock',
    });
    ...
    this.nodes.lowerUpper = new THREE.Vector3(lowerX, lerp(hallFloorY, descEndY - 1.05, 0.22), lerp(hallSouthZ, descEndZ, 0.22));
    this.nodes.lowerMid = new THREE.Vector3(lowerX, lerp(hallFloorY, descEndY - 1.05, 0.6), lerp(hallSouthZ, descEndZ, 0.6));
    ...
    // Stops at the north wall of the hall at its head; the last four metres of
    // the surveyed run are that room.
    const ascStopZ = ascEndZ - 4.2;
    const ascStopY = ascEndY - 4.2 * Math.tan(SLOPE);
    buildPassage(lime, colliders, ascStartY, hallSouthZ, ascStopY, ascStopZ, PW, PH);
    this._lineTorches(ascStartY, hallSouthZ, ascStopY, ascStopZ, 8);
    ...
    /**
     * The head of the Ascending Passage is the second knot: the Grand Gallery
     * and the horizontal passage to the Queen's Chamber both begin here, at
     * the same floor height, in a corridor a metre wide.  Whichever the
     * collision world's step-up reached first won, and the Queen's Chamber
     * could not be reached on foot at all.
     *
     * Same answer as the junction below: make the meeting a room.  The way to
     * the Queen's Chamber leaves the south wall on the west side at floor
     * level; the gallery's foot is two metres up, reached by a flight against
     * the east side.  The gallery itself does not move, because the King's
     * Chamber is surveyed from it.
     */
    const topRun = 4.2;
    const topFloorY = ascEndY - topRun * Math.tan(SLOPE);
    const topHallZ = ascEndZ - topRun / 2;
    const topNorthZ = ascEndZ - topRun;
    const topHallCx = AXIS_X + 0.4;
    const qcX = AXIS_X + 1.9;

    buildRoom(
      dressed, colliders, topHallCx, topFloorY, topHallZ, 7.2, topRun, 3.4,
      [
        { wall: 'n', offset: AXIS_X - topHallCx, width: PW, height: PH + 0.6, sill: 0 },
        { wall: 's', offset: qcX - topHallCx, width: PW, height: PH + 0.6, sill: 0 },
        { wall: 's', offset: AXIS_X - topHallCx, width: gg.bottomWidth + 0.3, height: 2.6, sill: ascEndY - topFloorY },
      ],
      { uvUnit: 1.8 }
    );
    // The flight up into the gallery, against the east side so the way west to
    // the Queen's Chamber stays clear.  Four treads inside the 0.55 m limit.
    for (let i = 0; i < 4; i++) {
      const top = topFloorY + ((ascEndY - topFloorY) / 4) * (i + 1);
      dressed.push(box(1.5, 0.53, topRun - 0.5 - i * 0.8, AXIS_X - 0.95, top - 0.265, topHallZ + 0.25 + i * 0.4));
      colliders.push({
        minX: AXIS_X - 1.7, maxX: AXIS_X - 0.2,
        minY: top - 0.7, maxY: top,
        minZ: topNorthZ + 0.4 + i * 0.8, maxZ: ascEndZ,
        tag: 'gallery-step',
      });
    }
    // Bedrock under this knot too, stopping dead at the hall's north wall: the
    // Ascending Passage climbs to meet it there, and a slab carried any
    // further north would sit above that floor and wall the passage off.
    // Two slabs rather than one: west of the Ascending Passage the rock can run
    // north past the hall, but under the passage itself it must stop at the
    // hall's wall, or it would sit above the climbing floor and wall it off.
    colliders.push({
      minX: AXIS_X - 2.2, maxX: topHallCx + 5.0,
      minY: topFloorY - 2.4, maxY: topFloorY - 0.02,
      minZ: topNorthZ, maxZ: ascEndZ + 1.5, tag: 'bedrock',
    });
    // West of the Ascending Passage and the gallery there is nothing but rock,
    // and nothing should ever stand there - but a seam between two shells is
    // easy to miss and the consequence underground is a fall with no bottom.
    // One slab covers the whole band. It clears the lower Descending Passage,
    // which runs thirty metres below it and a little further east.
    colliders.push({
      minX: AXIS_X - 10, maxX: AXIS_X - 2.2,
      minY: topFloorY - 2.4, maxY: topFloorY - 0.02,
      minZ: topNorthZ - 6, maxZ: ascEndZ + 14, tag: 'bedrock',
    });
    this.nodes.topHall = new THREE.Vector3(topHallCx, topFloorY, topHallZ);
    this.viewpoints.topHall = { position: this.nodes.topHall.clone(), yaw: Math.PI };
    this.torchSites.push({ x: topHallCx + 2.6, y: topFloorY + 2.0, z: topHallZ, scale: 0.8 });

    // ------------------------------------------------------ queen's chamber
    const qc = K.queensChamber;
    const qcNorthWallZ = -qc.d / 2;
    // Up out of the hall, then level all the way to the chamber.
    buildPassage(lime, colliders, topFloorY, ascEndZ, qc.floorY, ascEndZ + 6.0, PW, PH + 0.6, { x: qcX });
    buildPassage(lime, colliders, qc.floorY, ascEndZ + 6.0, qc.floorY, qcNorthWallZ, PW, PH + 0.6, { x: qcX });
    this._lineTorches(qc.floorY, ascEndZ + 6.0, qc.floorY, qcNorthWallZ, 9, qcX);

    buildRoom(
      dressed, colliders, AXIS_X, qc.floorY, 0, qc.w, qc.d, qc.wallH,
      [{ wall: 'n', offset: qcX - AXIS_X, width: PW, height: PH + 0.6, sill: 0 }],
    ...
    this.nodes.horizontal = new THREE.Vector3(qcX, qc.floorY, ascEndZ + 12);
  ...
  _decorateKhufu() {
    ...
    if (qc) {
      R.kaStatue(AXIS_X + K.queensChamber.w / 2 - 0.75, qc.y, qc.z + 1.7, -Math.PI * 0.5, {
        site: 'khufu',
        scale: 0.62,
        id: 'relic-khufu-ka',
      ...
      R.offeringTable(AXIS_X - K.queensChamber.w / 2 + 1.2, qc.y, qc.z + 0.2, Math.PI * 0.5, { site: 'khufu' });
      R.canopicChest(AXIS_X - K.queensChamber.w / 2 + 1.2, qc.y, qc.z + 1.9, Math.PI * 0.5, { site: 'khufu' });
    ...
    for (let i = 0; i < steps; i++) {
      ...
      for (const side of [-1, 1]) {
        colliders.push({
          ...
          // Only the west wall reaches down: the hall at the gallery's foot
          // sits a metre below it and left open air underneath. The east wall
          // must not, because the passage to the Queen's Chamber runs past it
          // at that depth.
          minY: Math.min(yA, yB) - (side < 0 ? 3.0 : 0.6),
          maxY: Math.max(yA, yB) + gg.height + 1.5,
